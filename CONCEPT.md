# MANTAPS Topology Engine — Concept & Design

> This is the original design document. For the current state (what runs, what
> does not, future works) see [`README.md`](README.md). Kept here because the
> analytical-context breakdown, the "why this exists" rationale, and the full
> model design below are still the reference for where the engine is headed.
>
> Where this document says "starter renderer" or "planned", check `README.md`
> for what has since been built (the SLD renderer, `/ingest`, `/editor`,
> the Tier engine, three real subsystem slices).
